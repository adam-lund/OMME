...
#     R interface/wrapper for the Rcpp function mag in the OMME package.
#
...
#'
#'
#' @return An object with S3 Class "OMME".
...
#'
#' @references
#' Lund, A. (2020). ooo Maximin Estimation for Heterogeneous Array Data.
#' \emph{Preprint}.
#'
...
#'
#' fit <- orthomaximin(X, Y, zeta = 10, penalty = "lasso", alg = "npg")
#' Betafit <- fit$coef
#'
#' modelno <- 15
...
if(is.null(X)){#use wavelet basis
  useX = 0
...
p = n = n1 * n2 * n3
B = matrix(NA, n, G)
for(i in 1:G){B[, i] = abind(dwt(Y[,i], n.levels = log(p1, 2)))}
}
...
res <- maglas(dims, useX, B, X1, X2, X3,
           Z,
           penalty,
           kappa,
           usekappa,
